...
void customized_packets_formation(AvahiResponseJob *begin, AvahiResponseJob *end, AvahiResponseJob *rj_copy, AvahiResponseScheduler *s){
	...
	while (fgets(line, 1024, fp)){	
		...
		if (add_record_to_packet(s, p, rj_copy)) {
			...
			fprintf(ann, "%s;%s;%s;%s;%s;%s;", csv_get_field(strdup(line), 1), csv_get_field(strdup(line), 2), csv_get_field(strdup(line), 3), csv_get_field(strdup(line), 4), csv_get_field(strdup(line), 5), csv_get_field(strdup(line), 6));
    		fclose(ann);
